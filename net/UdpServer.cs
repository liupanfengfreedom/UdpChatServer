...
namespace ChatServer
{
    ...
    class UdpServer
    {
        ...
        Dictionary<EndPoint, UdpChannelManager> UdpClientMap ;
        public UdpChannelManager findclient(EndPoint ep)
        {
        ...
        public UdpServer(IPAddress ipAd, int port)
        {
            ...
            UdpClientMap = new Dictionary<EndPoint, UdpChannelManager>();
        }
        ~ UdpServer()
        {
        ...
        void Listenerthreadwork()
        {
            ...
            while (true)
            {
                try
                {
                    ...
                    if (SuperUdpClient.isvalidedata(ref receivedbuffer))
                    {
                        byte[] validbuffer = new byte[bytesReceived];
                        Array.ConstrainedCopy(receivedbuffer, 0, validbuffer, 0, bytesReceived);
                        findclient(remoteEP).mudpclient.receivecallback(ref validbuffer);
                    }
                }
                catch (Exception e)
                {
